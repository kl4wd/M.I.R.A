...
int main() {
  ...
  for (int i = 0; i < numDevices; i++) {
    ...
    if (deviceInfo->maxInputChannels > 0 &&
        strstr(deviceInfo->name, "USB") != NULL) {
      usbDeviceIndex = i;
    }
  }
  printf("----------------------------\n");
  ...
  if (usbDeviceIndex != -1) {
    printf("%s[SUCCÈS] Micro USB trouvé à l'ID PortAudio : %d%s\n", COLOR_GREEN,
           usbDeviceIndex, COLOR_RESET);
    inputParameters.device = usbDeviceIndex;
  } else {
    printf("%s[ATTENTION] Pas de 'USB' dans le nom. Essai avec le périphérique "
           "par défaut...%s\n",
           COLOR_YELLOW, COLOR_RESET);
  ...
  if (err != paNoError) {
    fprintf(stderr, "%s[ERREUR] Pa_OpenStream: %s%s\n", COLOR_RED,
            Pa_GetErrorText(err), COLOR_RESET);
    printf("Essai de changer le SAMPLE_RATE dans le code si 44100 ne marche "
           "pas.\n");
    cleanup();
    return -1;
  }
